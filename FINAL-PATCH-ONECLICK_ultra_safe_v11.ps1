# FINAL-PATCH-ONECLICK (ULTRA SAFE) — v11
# Keeps: localhost scrub, remove srcset/sizes, emoji cleanup, UTF-8 (no BOM).
# Does NOT remove any Elementor JS.
# Injects CSS visibility guards + tablet-only fixes for iphone.webp.
# Removes tablet-hide classes if present.
# NEW: Writes an external JS helper (/assets/js/nm-tablet-fix.js) and injects it defer to fix tablet containers without inline JS (CSP-safe).
param([string]$Root='.')
...
$ts=0; $tc=0; $lf=0; $sr=0; $err=0; $guard=0; $norm=0; $casefix=0; $iphoneFix=0; $tabletUnhide=0; $jsInjected=0
...
# Prepare external JS content (safe, no inline)
$jsContent = @"
(()=>{
...
# Ensure /assets/js exists and write the helper
$assetsDir = Join-Path $Root 'assets/js'
if (!(Test-Path -LiteralPath $assetsDir)) {
  New-Item -ItemType Directory -Force -Path $assetsDir | Out-Null
}
$jsPath = Join-Path $assetsDir 'nm-tablet-fix.js'
[IO.File]::WriteAllText($jsPath, $jsContent, $utf8)

# Regex to inject external JS before </body> (or end of head as fallback)
$reBodyClose = [regex]'(?is)</body>'
$scriptTag = '<script defer src="/assets/js/nm-tablet-fix.js"></script>'
...
foreach ($f in $files) {
  try {
    ...
    if ($f.Extension -match '^\.(html|htm)$') {
      ...
      # Inject external JS once per page
      if ($new -notmatch 'nm-tablet-fix.js') {
        if ($reBodyClose.IsMatch($new)) {
          $new = $reBodyClose.Replace($new, $scriptTag + '</body>', 1)
        } else {
          $new = $reHeadClose.Replace($new, $scriptTag + '</head>', 1)
        }
        if ($new -ne $orig) { $jsInjected++ }
      }
    }

    # Case-fix filenames under uploads
    $matches = $reUploadsPath.Matches($new)
    if ($matches.Count -gt 0) {
      $unique = @{}; foreach ($m in $matches) { $unique[$m.Groups[1].Value] = $true }
      foreach ($relPath in $unique.Keys) {
        $diskPath = Join-Path $Root ($relPath.TrimStart('/').Replace('/', [IO.Path]::DirectorySeparatorChar))
        $dir = Split-Path $diskPath -Parent; $name = Split-Path $diskPath -Leaf
    ...
    if ($cIph -gt 0) {
      ...
      $new = $reIphoneImg.Replace($new, { param($m)
          ...
          if ($pre -notmatch '(?i)\bloading\s*=' -and $post -notmatch '(?i)\bloading\s*=') { $post = $post + ' loading="eager"' }
      ...
      # Remove tablet-hide classes in this file
...
$report = @(
  'FINAL-PATCH-ONECLICK finished. (ULTRA SAFE v11)',
  ...
  ('External tablet JS injected           : {0}' -f $jsInjected),
  ('Errors                                : {0}' -f $err),
